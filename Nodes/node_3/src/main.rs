...
const NODE_3_ADDRESS: &str = "localhost:3336";
...
fn send_message_to_nodes(message: String) -> Result<(), Error> {
    ...
    active_addresses.retain(|address| address != NODE_3_ADDRESS);
...
fn main() -> io::Result<()> {
    ...
    let listener = TcpListener::bind(NODE_3_ADDRESS)?;
    println!("Server listening on port 3336...");
